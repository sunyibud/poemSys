...
public class SysMessage extends BaseEntity
{
    ...
    private long otherUserId;  //当类型newFans时表示粉丝的id
                                //当类型为letter时表示与之私信的用户id
                                //当类型为comment时表示对自己的评论进行回复的用户id
                                //当类型为likeOrCollect时表示操作的用户
    private boolean isShow;   //当类型为letter时表示是否在用户消息列表中显示
    private LocalDateTime receiveTime; //消息接受的时间
    private String uuid;    //消息的唯一标识
    private long beReplyCommentId; //评论类型时代表自己哪条评论被别人评论
}
